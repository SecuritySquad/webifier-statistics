...
p <- plot_ly(data=result,y=result$durationInMillis,x=result$overallResultValue,color=result$overallResultType,colors=COL.LIST)  %>%
  layout(xaxis = list(title="Ergebniswert"), yaxis = list(title="Zeit in Sekunden"))
ggplotly(p)
```
...
p <- plot_ly(data=result,y=result$durationInMillis,x=result$X_id,color=result$result,colors=COL.LIST)  %>%
  layout(xaxis = list(title="Ergebnis",showticklabels=FALSE), yaxis = list(title="Zeit in Sekunden"))
ggplotly(p)
```
