...
result <- result[result$TLD==result.size[result.size$id>200,]$TLD,]
...
fin.result <- as.data.frame(cbind(levels(factor(result$TLD))))
...
fin.result <- merge(fin.result,result[result$Ergebnis=='unbekannt',c(-2,-3)],by="TLD",all=TRUE)
names(fin.result) <- c('TLD','bedrohlich','verdächtig','unbekannt')
fin.result[is.na(fin.result$bedrohlich),2] <- 0
fin.result[is.na(fin.result$verdächtig),3] <- 0
fin.result[is.na(fin.result$unbekannt),4] <- 0
...
p <- plot_ly(fin.result, x = ~TLD, y = ~bedrohlich, type = 'bar', name = 'bedrohlich', marker = list(color=COL.MALICIOUS)) %>%
  ...
  add_trace(y = ~unbekannt, name = 'unbekannt', marker = list(color=COL.UNDEFINED)) %>%
