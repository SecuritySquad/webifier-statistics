...
testResults[resultType$overallResultType=='MALICIOUS',2] <- "bedrohlich"
testResults[resultType$overallResultType=='SUSPICIOUS',2] <- "verdächtig"
testResults[resultType$overallResultType=='CLEAN',2] <- "sauber"
testResults[resultType$overallResultType=='UNKNOWN',2] <- "unbekannt"
names(testResults) <- c('Testname','Ergebnis')
p <- ggplot(data = testResults, aes(x = Testname, fill = Ergebnis)) +
            geom_bar(position = "dodge") +
            ylab("Anzahl") + 
  scale_fill_manual(values=c("red","darkgreen","orange","grey"))
ggplotly(p)
...
result[result$overallResultType=='UNKNOWN',2] <- "unbekannt"
