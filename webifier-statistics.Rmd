...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierTestResultData',"{}", "{durationInMillis:1,overallResultType:1,overallResultValue:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$overallResultType),]
p <- plot_ly(data=result,y=result$durationInMillis,x=result$overallResultValue,color=result$overallResultType,colors=COL.LIST)  %>%
  layout(xaxis = list(title="Ergebniswert",range=list(0,1.1)), yaxis = list(title="Zeit in Sekunden"))
ggplotly(p)
```
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'VirusScan'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
result[result$result=="SUSPICIOUS",3]<-2
result[result$result=="MALICIOUS",3]<-3
result$X_id <- as.numeric(result$X_id)
p <- plot_ly(data=result,y=result$durationInMillis,x=result$X_id,color=result$result,colors=COL.LIST)  %>%
  layout(xaxis = list(title="Ergebnis",showticklabels=FALSE,range=list(0,4)), yaxis = list(title="Zeit in Sekunden"))
ggplotly(p)
```
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'PortScan'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'IpScan'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'PhishingDetector'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'HeaderInspection'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'LinkChecker'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'GoogleSafeBrowsing'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
...
### Verhältnis von Zeit zu Testergebnis
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'CertificateChecker'}", "{durationInMillis:1,result:1}",skip=0,limit=Inf)
result$durationInMillis <- result$durationInMillis/1000
result<-result[order(result$result),]
result[result$result=="UNDEFINED",3]<-0
result[result$result=="CLEAN",3]<-1
