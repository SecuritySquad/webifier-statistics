...
dygraph(xts) %>%
  ...
  dySeries("clexts", label = "sauber",color=COL.CLEAN) %>%
...
Hier kommen noch mehr Benchmarks
...
result[,1] <- tld_extract(result[,1],NULL)[,2]
result <- result[result[,2]!="UNDEFINED",]
result[result$overallResultType=='CLEAN',2] <- "sauber"
result[result$overallResultType=='MALICIOUS',2] <- "bedrohlich"
result[result$overallResultType=='SUSPICIOUS',2] <- "verdächtig"
...
fin.result <- merge(fin.result,result[result$Ergebnis=='sauber',c(-2,-3)],by="TLD",all=TRUE)
fin.result <- merge(fin.result,result[result$Ergebnis=='sauber',c(-2,-3)],by="TLD",all=TRUE)
names(fin.result) <- c('TLD','bedrohlich','verdächtig','sauber','order')
fin.result[is.na(fin.result$bedrohlich),2] <- 0
fin.result[is.na(fin.result$verdächtig),3] <- 0
fin.result[is.na(fin.result$sauber),4] <- 0
fin.result$order <- fin.result$bedrohlich+0.01*fin.result$verdächtig
fin.result$sauber <- 100-fin.result$bedrohlich-fin.result$verdächtig
fin.result <- fin.result[fin.result$sauber!=100,]
...
p <- plot_ly(fin.result, x = ~TLD, y = ~bedrohlich, type = 'bar', name = 'bedrohlich', marker = list(color=COL.MALICIOUS)) %>%
  ...
  add_trace(y = ~sauber, name = 'sauber', marker = list(color=COL.CLEAN)) %>%
...
### Gefundene Viren
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'VirusScan'}", '{resultInfo:1, result:1}',skip=0,limit=Inf)
#TODO
```
...
format <- "%a %b %d %H:%M:%S CEST %Y"
result[,1] <- format(as.Date(as.POSIXct(result[,1],format, tz="CET")),"%d-%m-%Y")
result[result$Ergebnis=="MALICIOUS",3] <- "bedrohlich"
result[result$Ergebnis=="CLEAN",3] <- "sauber"
result[result$Ergebnis=="SUSPICIOUS",3] <- "verdächtig"
result[result$Ergebnis=="UNDEFINED",3] <- "unbekannt"
#datatable(result, options = list(pageLength = 100), rownames=FALSE)
```
Coming soon...
