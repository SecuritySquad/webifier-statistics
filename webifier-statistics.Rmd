...
result <- result[result$overallResultType!='CLEAN',]
...
result <- aggregate(val ~ country + iso3c,result,sum)
...
###Some Auswertung stuff
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'PhishingDetector'}", '{resultInfo:1,result:1}',skip=0,limit=Inf)
test <- result[1,1]
```
...
###Some Auswertung stuff
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'HeaderInspection'}", '{resultInfo:1,result:1}',skip=0,limit=Inf)
test <- result[1,1]
```
...
###Some Auswertung stuff
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'LinkChecker'}", '{resultInfo:1,result:1}',skip=0,limit=Inf)
test <- result[1,1]
```
...
###Some Auswertung stuff
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'GoogleSafeBrowsing'}", '{resultInfo:1,result:1}',skip=0,limit=Inf)
test <- result[1,1]
```
...
###Some Auswertung stuff
```{r}
result <- dbGetQueryForKeys(mg1, 'webifierSingleTestResultData',"{name:'CertificateChecker'}", '{resultInfo:1,result:1}',skip=0,limit=Inf)
test <- result[1,1]
```
